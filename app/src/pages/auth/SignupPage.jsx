...
export default function SignupPage() {
  ...
  const [selectedRole, setSelectedRole] = useState(null) // 'employee' | 'employer'
  const [error, setError] = useState(null)
  const [isCreatingUser, setIsCreatingUser] = useState(false)

  // Get the intended destination after signup
  const from = location.state?.from?.pathname || (selectedRole === 'employer' ? '/company/profile' : '/dashboard/profile')
  ...
  useEffect(() => {
    ...
    handlePostAuth()
  }, [user, userRole, selectedRole, isCreatingUser, createUserRecord, navigate])
